...
const fetchJobs = async (syncLogger) => {
  const configPath = path.join(__dirname, '../config/connectors/ashby.json');
  let companies = [];
  try {
  ...
  const promises = companies.map(company => limit(async () => {
    ...
    const result = await fetchWithRetry(`https://api.ashbyhq.com/posting-api/job-board/${companyTrimmed}`);
