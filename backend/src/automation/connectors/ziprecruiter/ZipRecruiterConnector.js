...
class ZipRecruiterConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[ZipRecruiterConnector] Initializing ZipRecruiter plugin...');
    this.logOwnership('ConnectorInitialization', 'ZipRecruiterConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'ZipRecruiterConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'ZipRecruiter_Upload' });
    }
  }
...
module.exports = ZipRecruiterConnector;
