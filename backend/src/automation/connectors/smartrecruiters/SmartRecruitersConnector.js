...
class SmartRecruitersConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[SmartRecruitersConnector] Initializing SmartRecruiters plugin...');
    this.logOwnership('ConnectorInitialization', 'SmartRecruitersConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'SmartRecruitersConnector');
    await this.page.goto(jobUrl, { waitUntil: 'domcontentloaded' });
  }

  async detectApplication() {
    const applyBtn = this.page.locator('button[id*="st-apply"], a[href*="apply"]').first();
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'SmartRecruiters_Upload' });
    }
  }
  ...
  async review() {}
  async submit() {
    const submitBtn = this.formContext.locator('button[type="submit"], button#st-submit').first();
...
module.exports = SmartRecruitersConnector;
