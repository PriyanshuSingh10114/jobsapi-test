...
class PersonioConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[PersonioConnector] Initializing Personio ATS plugin...');
    this.logOwnership('ConnectorInitialization', 'PersonioConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'PersonioConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'Personio_Upload' });
    }
  }
