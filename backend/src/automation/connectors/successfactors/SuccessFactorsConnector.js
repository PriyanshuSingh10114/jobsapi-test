...
class SuccessFactorsConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[SuccessFactorsConnector] Initializing SAP SuccessFactors ATS plugin...');
    this.logOwnership('ConnectorInitialization', 'SuccessFactorsConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'SuccessFactorsConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'SuccessFactors_Upload' });
    }
  }
...
module.exports = SuccessFactorsConnector;
