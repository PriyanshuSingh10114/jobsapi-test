...
class WorkableConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[WorkableConnector] Initializing Workable ATS plugin...');
    this.logOwnership('ConnectorInitialization', 'WorkableConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'WorkableConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'Workable_Upload' });
    }
  }
...
module.exports = WorkableConnector;
