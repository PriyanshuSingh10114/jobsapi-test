...
class WellfoundConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[WellfoundConnector] Initializing Wellfound (AngelList) ATS plugin...');
    this.logOwnership('ConnectorInitialization', 'WellfoundConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'WellfoundConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'Wellfound_Upload' });
    }
  }
...
module.exports = WellfoundConnector;
