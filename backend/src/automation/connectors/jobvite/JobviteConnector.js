...
class JobviteConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[JobviteConnector] Initializing Jobvite ATS plugin...');
    this.logOwnership('ConnectorInitialization', 'JobviteConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'JobviteConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'Jobvite_Upload' });
    }
  }
  ...
  async review() {}
  async submit() {
    const submitBtn = this.formContext.locator('input[type="submit"], button.jv-button').first();
...
module.exports = JobviteConnector;
