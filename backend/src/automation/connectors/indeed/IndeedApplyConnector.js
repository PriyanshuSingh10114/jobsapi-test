...
class IndeedApplyConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[IndeedApplyConnector] Initializing Indeed Apply plugin...');
    this.logOwnership('ConnectorInitialization', 'IndeedApplyConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'IndeedApplyConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'Indeed_Upload' });
    }
  }
...
module.exports = IndeedApplyConnector;
