...
class DiceConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[DiceConnector] Initializing Dice ATS plugin...');
    this.logOwnership('ConnectorInitialization', 'DiceConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'DiceConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'Dice_Upload' });
    }
  }
