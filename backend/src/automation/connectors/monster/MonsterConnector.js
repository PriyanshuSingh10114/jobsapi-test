...
class MonsterConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[MonsterConnector] Initializing Monster ATS plugin...');
    this.logOwnership('ConnectorInitialization', 'MonsterConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'MonsterConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'Monster_Upload' });
    }
  }
...
module.exports = MonsterConnector;
