...
class PinpointConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[PinpointConnector] Initializing Pinpoint ATS plugin...');
    this.logOwnership('ConnectorInitialization', 'PinpointConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'PinpointConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'Pinpoint_Upload' });
    }
  }
...
module.exports = PinpointConnector;
