...
class BambooHRConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[BambooHRConnector] Initializing BambooHR ATS plugin...');
    this.logOwnership('ConnectorInitialization', 'BambooHRConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'BambooHRConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'BambooHR_Upload' });
    }
  }
...
module.exports = BambooHRConnector;
