...
class OracleRecruitingConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[OracleRecruitingConnector] Initializing Oracle Recruiting Cloud ATS plugin...');
    this.logOwnership('ConnectorInitialization', 'OracleRecruitingConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'OracleRecruitingConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'Oracle_Upload' });
    }
  }
...
module.exports = OracleRecruitingConnector;
