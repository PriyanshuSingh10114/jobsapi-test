...
class USAJobsConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[USAJobsConnector] Initializing USAJobs ATS plugin...');
    this.logOwnership('ConnectorInitialization', 'USAJobsConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'USAJobsConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'USAJobs_Upload' });
    }
  }
...
module.exports = USAJobsConnector;
