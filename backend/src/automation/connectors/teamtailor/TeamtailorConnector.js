...
class TeamtailorConnector extends BaseApplicationConnector {
  async initialize() {
    logger.info('[TeamtailorConnector] Initializing Teamtailor ATS plugin...');
    this.logOwnership('ConnectorInitialization', 'TeamtailorConnector');
  ...
  async openJob(jobUrl) {
    this.logOwnership('OpeningJob', 'TeamtailorConnector');
  ...
  async uploadResume(profileData) {
    ...
    if (await fileInput.count() > 0) {
      ...
      this.uploadResults.push({ type: 'Resume', verified: true, path: resumePath, method: 'Teamtailor_Upload' });
    }
  }
...
module.exports = TeamtailorConnector;
